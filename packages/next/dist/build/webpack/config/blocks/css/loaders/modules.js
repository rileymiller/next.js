...
function getCssModuleLoader(ctx, postcss, preProcessors = []) {
    ...
    loaders.push({
        ...
        options: {
            ...
            esModule: false,
            url: (url, resourcePath)=>(0, _fileResolve).cssFileResolve(url, resourcePath, ctx.experimental.urlImports, ctx.experimental.cssFileResolveIgnoreUrlPattern)
            ,
            import: (url, _, resourcePath)=>(0, _fileResolve).cssFileResolve(url, resourcePath, ctx.experimental.urlImports, ctx.experimental.cssFileResolveIgnoreUrlPattern)
            ,
            modules: {
